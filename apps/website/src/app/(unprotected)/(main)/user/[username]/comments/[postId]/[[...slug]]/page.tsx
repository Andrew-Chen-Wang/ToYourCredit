...
export async function generateMetadata({
  ...
  const description = post.bodyMd?.slice(0, 200) ?? `A post by u/${username} on ToYourCredit.`
...
/** Anonymous (SSR) detail page for a profile post: /user/:username/comments/:id[/:slug]. */
export default async function ProfilePostDetailPage({
  ...
  const profile = await fetchUser(db).getOneByUsername(username, ["id", "username"])
  if (!profile) {
    notFound()
  }
    ...
    <div className="mx-auto mt-4 flex w-full max-w-5xl flex-col gap-6 px-4 pb-10 lg:flex-row">
      <div className="min-w-0 flex-1">
        ...
        <AnonCommentSection
          postId={postId}
          communityName=""
          permalinkBase={`/user/${profile.username}/comments/${postId}`}
          sort={sort}
          nodes={commentNodes}
